...
import com.StreamScape.engine.task.impl.FamiliarSpawnTask;
import com.StreamScape.model.GameMode;
import com.StreamScape.model.Gender;
...
import com.StreamScape.model.Prayerbook;
import com.StreamScape.model.container.impl.Bank;
import com.StreamScape.net.login.LoginResponses;
import com.StreamScape.world.content.ColourSet;
import com.StreamScape.world.content.KillsTracker;
...
import com.StreamScape.world.content.skill.SkillManager.Skills;
...
public class PlayerLoading {
	...
	public static int getResult(Player player) {
		...
		try (FileReader fileReader = new FileReader(file)) {
			...
			if (reader.has("pet")) {
				int pet = reader.get("pet").getAsInt();
				if (pet != -1) {
					TaskManager.submit(new Task(5) {
						@Override
						protected void execute() {
							CustomPet.spawnForItem(player, pet, false);
							player.getPacketSender().sendMessage("your pet is loyal enough to come back with you!");
							stop();
						}
					});
				}
			...
			if (reader.has("extradroprate")) {
				player.setExtraDropRate(reader.get("extradroprate").getAsInt());
			}
