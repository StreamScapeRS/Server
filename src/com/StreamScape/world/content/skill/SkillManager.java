...
import com.StreamScape.world.content.BrawlingGloves;
	...
	public SkillManager addExperience(Skill skill, int experience) {
		...
		if (newLevel > startingLevel) {
			...
			if (skills.maxLevel[skill.ordinal()] == getMaxAchievingLevel(skill)) {
				...
				World.sendMessage("<img=15> @red@News: @bla@" + player.getUsername()
						+ " @red@has just achieved the highest possible level in " + skillName + "!");
			...
			if (getCombatLevel() > 50) {
				Achievements.doProgress(player, AchievementData.COMBAT_LVL_50);
			}
			if (getCombatLevel() > 75) {
				Achievements.doProgress(player, AchievementData.COMBAT_LVL_75);
			}
			if (getCombatLevel() > 100) {
				Achievements.doProgress(player, AchievementData.COMBAT_LVL_100);
			}
