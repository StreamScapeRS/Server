package com.StreamScape.world.content.dialogue;
...
import com.StreamScape.model.input.impl.BuyShards;
import com.StreamScape.model.input.impl.ChangePassword;
import com.StreamScape.model.input.impl.DonateToWell;
import com.StreamScape.model.input.impl.SellShards;
import com.StreamScape.model.input.impl.SetEmail;
...
import com.StreamScape.world.content.Artifacts;
import com.StreamScape.world.content.BankPin;
import com.StreamScape.world.content.CustomObjects;
import com.StreamScape.world.content.DonorTickets;
import com.StreamScape.world.content.Gambling.FlowersData;
import com.StreamScape.world.content.Lottery;
import com.StreamScape.world.content.LoyaltyProgram;
import com.StreamScape.world.content.PkSets;
import com.StreamScape.world.content.PlayerPanel;
import com.StreamScape.world.content.Scoreboards;
import com.StreamScape.world.content.WellOfGoodwill;
import com.StreamScape.world.content.clan.ClanChatManager;
import com.StreamScape.world.content.dialogue.impl.AgilityTicketExchange;
import com.StreamScape.world.content.dialogue.impl.Mandrith;
...
import com.StreamScape.world.content.skill.impl.dungeoneering.DungeoneeringFloor;
		...
		if (id == FIRST_OPTION_OF_FIVE) {
			switch (player.getDialogueActionId()) {
				...
				case 15001:
					...
					player.getUpdateFlag().flag(Flag.APPEARANCE);
					player.getPacketSender().sendInterfaceRemoval();
					player.sendMessage("<col=ff0000>You transformed into the StreamScape Point Store.");
					break;
				case 15002:
			...
			switch (player.getDialogueActionId()) {
				...
				case 15000:
					...
					player.getUpdateFlag().flag(Flag.APPEARANCE);
					player.getPacketSender().sendInterfaceRemoval();
					player.sendMessage("<col=ff0000>You transformed into a Purgatory.");
					break;
				case 15001:
				...
				case 5:
					...
					if (player.getRights() == PlayerRights.PLAYER) {
						player.getPacketSender().sendMessage("You need to be a member to teleport to this zone.")
								.sendMessage("To become a member, visit StreamScape.com and purchase a scroll.");
						return;
					}
			...
			switch (player.getDialogueActionId()) {
				...
				case 45:
					player.getPacketSender().sendString(1,
							"wwww.StreamScape.com/forums/index.php?/topic/104-game-mode-guide/?p=420");
					break;
			}
			...
			switch (player.getDialogueActionId()) {
				...
				case 420:
					...
					break;
				case 1501:
					TeleportHandler.teleportPlayer(player, new Position(2832, 2829),
							player.getSpellbook().getTeleportType());
					break;

				case 1551:
					TeleportHandler.teleportPlayer(player, new Position(2023, 4660),
							player.getSpellbook().getTeleportType());
					break;
				case 128:
			...
			switch (player.getDialogueActionId()) {
				...
				case 1551:
					...
					int height = (Misc.random(100000)*100)+4;
					boolean usePouch = player.getMoneyInPouch() >= (long) 250000000 * 1000L;

					if (!usePouch && player.getInventory().getAmount(13664) < 250) {
						player.getPacketSender().sendMessage("You do not have enough Bucks.");
						return;
					}
					if (usePouch) {
						player.setMoneyInPouch(player.getMoneyInPouch() - 250000000 * 1000L);
						player.getPacketSender().sendString(8135, "" + player.getMoneyInPouch());
					} else
						player.getInventory().delete(13664, 250);

					player.getPacketSender().sendMessage("You have just entered the Ichigo instance.");
					TeleportHandler.teleportPlayer(player, (new Position(2023, 4660, height)),
							player.getSpellbook().getTeleportType());

					NPC ICHIGO = new NPC(666, new Position(2032,4662,height));
					World.register(ICHIGO);
					break;

				case 1501:
					height = (Misc.random(100000)*100)+4;
					usePouch = player.getMoneyInPouch() >= (long) 1500000000 * 100L;

					if (!usePouch && player.getInventory().getAmount(13664) < 150) {
						player.getPacketSender().sendMessage("You do not have enough Bucks.");
						return;
					}
					if (usePouch) {
						player.setMoneyInPouch(player.getMoneyInPouch() - 1500000000 * 100L);
						player.getPacketSender().sendString(8135, "" + player.getMoneyInPouch());
					} else
						player.getInventory().delete(13664, 150);

					player.getPacketSender().sendMessage("You have just entered the Corporeal instance.");
					TeleportHandler.teleportPlayer(player, (new Position(2832, 2829, height)),
							player.getSpellbook().getTeleportType());

					NPC Corp = new NPC(8133, new Position(2833,2843,height));
					World.register(Corp);
					break;
				case 4:
				case 16:
			...
			switch (player.getDialogueActionId()) {
				...
				case 71:
					...
					if (player.getInventory().contains(19670) && player.getClickDelay().elapsed(700)) {
						...
						player.getInventory().delete(19670, amt);
						player.getPointsManager().setWithIncrease("voting", amt);
