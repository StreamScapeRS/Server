...
import com.StreamScape.model.movement.PathFinder;
...
import com.StreamScape.world.content.ItemDegrading;
import com.StreamScape.world.content.ItemDegrading.DegradingItem;
...
import com.StreamScape.world.content.combat.effect.EquipmentBonus;
import com.StreamScape.world.content.combat.magic.CombatAncientSpell;
...
import com.StreamScape.world.content.combat.weapon.FightStyle;
...
public final class CombatFactory {
	...
	public static boolean checkAttackDistance(Character a, Character b) {
		...
		/*
		 * Clipping checks and diagonal blocking by StreamScape
		 */
	...
	public static Hit getHit(Character entity, Character victim, CombatType type) {
		int divideDamageBy = 1;
		if (entity.isPlayer()) {
			Player player = (Player)entity;
			if (World.minigameHandler.getMinigame(1).isInGame(player))
				divideDamageBy = 20;
		}
		switch (type) {
			case MELEE:
				return new Hit(
						Misc.inclusiveRandom(1, /* CombatFactory.calculateMaxMeleeHit(entity, victim) */ DesolaceFormulas
								.calculateMaxMeleeHit(entity, victim) / divideDamageBy),
						Hitmask.RED, CombatIcon.MELEE);
			case RANGED:
				return new Hit(Misc.inclusiveRandom(1, CombatFactory.calculateMaxRangedHit(entity, victim)/ divideDamageBy), Hitmask.RED,
						CombatIcon.RANGED);
			case MAGIC:
				return new Hit(Misc.inclusiveRandom(1, DesolaceFormulas.getMagicMaxhit(entity)/ divideDamageBy), Hitmask.RED,
						CombatIcon.MAGIC);
			case DRAGON_FIRE:
				return new Hit(Misc.inclusiveRandom(0, CombatFactory.calculateMaxDragonFireHit(entity, victim)/ divideDamageBy),
